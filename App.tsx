...
import { InvariantType, SolverResult, SolverLog, ReferenceProblem, TestSuiteReport, UserRequest } from './types';
import { 
...
const App: React.FC = () => {
  ...
  const [reqCategory, setReqCategory] = useState<UserRequest['category']>('Deduction Logic');
  ...
  useEffect(() => {
    ...
    const savedRequests = localStorage.getItem('axiom_prime_requests');
    if (savedRequests) {
      try {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        ...
        addLogs([{ timestamp: new Date().toLocaleTimeString(), type: 'info', message: 'Visual manifold loaded via file upload.' }]);
      };
  ...
  const handleRunDiagnostics = async () => {
    ...
    addLogs([{ timestamp: new Date().toLocaleTimeString(), type: 'info', message: 'Initiating Lumina Logic stress tests...' }]);
  ...
  const handleSubmitRequest = (e: React.FormEvent) => {
    ...
    setReqTitle('');
    setReqDesc('');
    addLogs([{ timestamp: new Date().toLocaleTimeString(), type: 'success', message: `Request "${reqTitle}" added to Lumina Queue.` }]);
  };
  ...
  const handleSolve = async () => {
    ...
    setAllLogs([{ timestamp: new Date().toLocaleTimeString(), type: 'info', message: `Initializing Lumina Logic traversal...` }]);
  ...
  const handleExportPDF = () => {
    ...
    doc.text("LUMINA INVARIANT RESEARCH REPORT", 20, 20);
    doc.setFontSize(10);
    ...
    doc.save(`Lumina_Research_${Date.now()}.pdf`);
        ...
        <div className="flex items-center gap-6">
          <div className="relative group">
            ...
            <div className="relative glass-panel p-5 rounded-3xl border border-yellow-500/30 glow-hover transition-all duration-500">
              <FastForward className="w-14 h-14 text-yellow-500" />
            </div>
          </div>
          <div>
            <h1 className="text-5xl font-black tracking-tighter gold-gradient leading-tight uppercase font-mono italic">
              Lumina Invariant
            </h1>
          ...
          <div className="absolute inset-0 z-50 glass-panel rounded-[2.5rem] p-12 flex flex-col gap-10 animate-in zoom-in-95 duration-300 overflow-y-auto terminal-scroll">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-12">
              ...
              <div className="md:col-span-4 space-y-8">
                <div className="glass-panel p-8 rounded-[2.5rem] bg-yellow-500/5 border-yellow-500/10 space-y-6">
                  <h4 className="text-[10px] font-black uppercase tracking-[0.5em] text-yellow-500">System Architecture</h4>
                  <div className="space-y-6 relative pl-6">
                    <div className="absolute left-1 top-2 bottom-2 w-px bg-yellow-500/20" />
                    {[
                      {i: <Camera className="w-4 h-4"/>, t: 'Optic Ingest', d: 'Visual pattern parsing.'},
                      {i: <Mic className="w-4 h-4"/>, t: 'Acoustic Feed', d: 'Natural language transcription.'},
                      {i: <Cpu className="w-4 h-4"/>, t: 'Invariant Check', d: 'Pattern match traversal.'},
                      {i: <Layers className="w-4 h-4"/>, t: 'Quantum Bridge', d: 'Gemini 3.0 High-Reasoning.'}
                    ].map((step, idx) => (
